...
class HelpActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        // ===== 四、操作集 =====
        container.addView(makeCard("操作集（Action Set）",
            "操作集位于操作层之上，是「公共层 + 10 个操作层」的完整配置集合。\n"
                ...
                + "· 悬浮窗展开面板顶部显示当前操作集名称\n"
                + "· 典型用法：为不同职业/场景准备独立操作集，如「默认」「治疗」「PVP」"))
        ...
        // ===== 六、层与操作集设置 =====
        container.addView(makeCard("层与操作集设置",
            "· 应用内：配置管理 → 层与操作集设置\n"
    ...
    private fun profileName(): String {
        return try {
            getCurrentProfile().activeActionSetName
        } catch (e: Exception) {
            ControllerProfile.DEFAULT_ACTION_SET_NAME
        }
    }
