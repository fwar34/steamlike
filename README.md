...
- **智能暂停**: 检测前台应用（UsageStats），仅当 Winlator 等白名单应用在前台时保持手柄捕获，切到其他应用自动移除焦点窗口，解决 Android 13+ 焦点悬浮窗导致右滑返回失效的问题
...
2. 保持白名单默认值 `com.winlator`（或按需增删包名，逗号分隔）
3. 生效逻辑：**Winlator 在前台 → 保持手柄捕获；切到其他应用 → 自动移除焦点窗口，右滑返回恢复正常；切回 Winlator → 自动恢复捕获**

注意：
- 未授权"使用情况访问"时自动退化为手动模式（悬浮窗"暂停捕获/恢复捕获"按钮）
- 关闭"智能暂停"开关即回到手动模式（悬浮窗按钮重新显示）
